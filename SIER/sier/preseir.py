...
worksheet.write('B1', '填充后的甲型流感样本数')
worksheet.write('C1', '未来预测甲型流感样本数')

# 写入数据
for i in range(len(t_future)):
    row = i + 1
    date = data['日期'].iloc[0] + pd.Timedelta(days=i) if i < len(t) else data['日期'].iloc[-1] + pd.Timedelta(days=(i - len(t)))
    filled_cases = cases_A_filled[i] if i < len(t) else None
    predicted_cases = future_predictions[i]
    worksheet.write(row, 0, date.strftime('%Y-%m-%d'))
    worksheet.write(row, 1, filled_cases)
    worksheet.write(row, 2, predicted_cases)
