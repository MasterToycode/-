...
plt.plot(t, fitted, '-', label='模型拟合数据', color='blue')
plt.xlabel('时间（天）')
plt.ylabel('样本数')
plt.title('实际甲型流感样本数与模型拟合数据对比')
plt.legend(loc='upper left')
plt.grid(True)
plt.xticks(np.arange(0, len(t), step=50))
plt.tight_layout()
#plt.savefig('fit_comparison.png')  # 保存对比图
plt.show()
...
plt.xlabel('时间（天）')
plt.ylabel('样本数')
plt.title('甲型流感感染人数预测及未来预测')
plt.xticks(np.arange(0, len(t_future), step=50))
plt.legend(loc='upper left')
plt.grid(True)
plt.tight_layout()
plt.savefig('future_predictions.png')  # 保存预测图
plt.show()
